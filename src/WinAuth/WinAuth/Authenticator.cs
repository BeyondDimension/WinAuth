    ...
    public static AuthenticatorValueDTO? ReadXmlv2(XmlReader reader, string? password = null)
    {
        ...
        if (!string.IsNullOrEmpty(authenticatorType))
        {
            authenticatorType = authenticatorType.Replace(
                ...
                typeof(AuthenticatorValueDTO).FullName + ".");
...
#pragma warning disable IL2072 // Target parameter argument does not satisfy 'DynamicallyAccessedMembersAttribute' in call to target method. The return value of the source method does not have matching annotations.
            authenticator = (AuthenticatorValueDTO?)Activator.CreateInstance(authenticatorType_);
#pragma warning restore IL2072 // Target parameter argument does not satisfy 'DynamicallyAccessedMembersAttribute' in call to target method. The return value of the source method does not have matching annotations.
    ...
    */
    ...
    static byte[] Protect(byte[] userData, byte[]? optionalEntropy, DataProtectionScope scope)
    {
        ...
        {
            return ProtectedData.Protect(userData, optionalEntropy, (System.Security.Cryptography.DataProtectionScope)scope);
        }
        else
        {
    ...
    static byte[] Unprotect(byte[] encryptedData, byte[]? optionalEntropy, DataProtectionScope scope)
    {
        if (OperatingSystem.IsWindows())
        {
            return ProtectedData.Unprotect(encryptedData, optionalEntropy, (System.Security.Cryptography.DataProtectionScope)scope);
        }
        else
        {
            var s = Ioc.Get_Nullable<ILocalDataProtectionProvider.IProtectedData>();
