...
public partial class SteamClient : IDisposable
{
    ...
    public async Task<(IEnumerable<string> receiveItems, IEnumerable<string> sendItems)> GetConfirmationItemImageUrls(string tradeId)
    {
        ...
        //TODO
        //if (jsonobj.Html.StartsWith("如要上架"))
        //{
        //    return (receiveItems, sendItems);
        //}
        ...
        if (jsonobj.Success == true)
        {
            ...
            foreach (Match items in masterRegex)
            {
                ...
                if (items.Groups[0].Value.Contains("您需要放弃") || items.Groups[0].Value.Contains("You will give up"))
                {
        ...
        CookieContainer? cookies = null, NameValueCollection? headers = null, int timeout = 0, bool isProtobuf = false)
    {
            ...
            foreach (var key in data.AllKeys)
            {
                ...
                if (values.Any_Nullable())
                {
                    foreach (var value in values)
                    {
                        ...
                        var keyValueString = string.Format("{0}={1}", encodedKey, encodedValue);
        ...
        {
            LogException(method, url, cookies, data, exception);

            if (exception.Response != null &&
                ((HttpWebResponse)exception.Response).StatusCode == HttpStatusCode.Forbidden)
                throw new WinAuthUnauthorisedRequestException(exception);
        }
        catch (Exception ex)
        {
    ...
    [GeneratedRegex("<div class=\"tradeoffer_items_header\">.*?<div class=\"tradeoffer_item_list\">(.*?)<div style=\"clear: ", RegexOptions.Singleline)]
